...
from .models import (
    UniteMesure, ConsommableType, Consommable, ConsommableConsomme,
    MaterielType, Materiel, MaterielStatus, MaterielStatusHistory,
    EssaimOrigin, EssaimRace, Essaim, EssaimStatus, EssaimStatusHistory,
    EssaimDetail, Comportement, EssaimSanteHistory,
    CadreType, HausseType, RucheType, LocalizationStatus, Localization,
    LocalizationStatusHistory, RucheStatus, Ruche, RucheStatusHistory,
    Hausse, HausseCadre, RucheHausseHistory
)
...
def localization_create(request):
    if request.method == 'POST':
        ...
        Localization.objects.create(name=name, description=description, created_at=timezone.now())
...
def hausse_type_create(request):
    ...
    if request.method == 'POST':
        ...
        HausseType.objects.create(name=name, cadre_max_capacity=cadre_max_capacity, cadre_type_id=cadre_type)
...
def hausse_cadre_list(request):
    cadres = HausseCadre.objects.select_related('hausse_id').order_by('-created_at')
...
def hausse_cadre_create(request):
    ...
    if request.method == 'POST':
        ...
        HausseCadre.objects.create(hausse_id=hausse, added_cadre=added_cadre, created_at=timezone.now())
...
def hausse_create(request):
    ...
    if request.method == 'POST':
        ...
        Hausse.objects.create(hausse_type_id=hausse_type, created_at=timezone.now())
...
def hausse_update(request, id):
    ...
    if request.method == 'POST':
        hausse_type_id = request.POST['hausse_type_id']
        hausse.hausse_type_id = get_object_or_404(HausseType, pk=hausse_type_id)
        hausse.save()
        return redirect('hausse_list')
...
def ruche_list(request):
    ruches = Ruche.objects.select_related('localizations_id', 'ruche_type_id', 'essaim_id')
...
def ruche_update(request, id):
    ...
    if request.method == 'POST':
        ...
        ruche_type_id = request.POST['ruche_type_id']
        essaim_id = request.POST['essaim_id']
        ruche.localizations_id = get_object_or_404(Localization, pk=localization_id)
...
def essaim_sante_history_list(request):
    histories = EssaimSanteHistory.objects.select_related('essaim_id', 'comportement_id').order_by('-created_at')
    return.claim(request, 'essaim_sante_histories/list.html', {'histories': histories})
...
def essaim_sante_history_update(request, id):
    ...
    if request.method == 'POST':
        ...
        history.comportement_id = get_object_or_404(Comportement, pk=comportement_id)
        history.save()
        return redirect('essaim_sante_history_list')
...
def essaim_list(request):
    essaims = Essaim.objects.select_related('essaim_origin_id', 'essaim_race_id')
...
def essaim_create(request):
    ...
    if request.method == 'POST':
        ...
        Essaim.objects.create(essaim_origin_id=origin, essaim_race_id=race, created_at=timezone.now())
...
def localization_status_history_list(request):
    histories = LocalizationStatusHistory.objects.select_related('localization_id', 'localization_status_id').order_by('-created_at')
...
def localization_status_history_create(request):
    ...
    if request.method == 'POST':
        ...
        LocalizationStatusHistory.objects.create(localization_id=localization, localization_status_id=status, created_at=timezone.now())
...
def localization_status_history_update(request, id):
    ...
    if request.method == 'POST':
        localization_id = request.POST['localization_id']
        status_id = request.POST['localization_status_id']
        history.localization_id = get_object_or_404(Localization, pk=localization_id)
        history.localization_status_id = get_object_or_404(LocalizationStatus, pk=status_id)
        history.save()
        return redirect('localization_status_history_list')
...
def ruche_hausse_history_list(request):
    histories = RucheHausseHistory.objects.select_related('ruche_id', 'hausse_id').order_by('-created_at')
...
def ruche_hausse_history_create(request):
    ...
    if request.method == 'POST':
        ...
        hausse = get_object_or_404(Hausse, pk=hausse_id)
        RucheHausseHistory.objects.create(ruche_id=ruche, hausse_id=hausse, is_removed=is_removed, created_at=timezone.now())
...
def ruche_status_history_list(request):
    histories = RucheStatusHistory.objects.select_related('ruche_id', 'ruche_status_id').order_by('-created_at')
...
def ruche_status_history_create(request):
    ...
    if request.method == 'POST':
        ...
        RucheStatusHistory.objects.create(ruche_id=ruche, ruche_status_id=status, created_at=timezone.now())
