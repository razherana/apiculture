...
def recolte_detail(request, pk):
    recolte = {'id': pk, 'created_at': '2023-07-15', 'ruche': {'description': 'Ruche A1 - Forêt'}, 'poids_miel': 25.5, 'qualite': 8, 'taux_humidite': 17.5, 'note': 'Miel très parfumé, récolte par temps sec.'}
...
def recolte_form(request):
    ruches = [{'id': 1, 'description': 'Ruche A1 - Forêt'}, {'id': 2, 'description': 'Ruche B2 - Prairie'}]
...
def historique_production(request):
    historique = [
        {'ruche__description': 'Ruche A1 - Forêt', 'total_poids': 120.5, 'qualite_moyenne': 8.2},
        {'ruche__description': 'Ruche B2 - Prairie', 'total_poids': 95.0, 'qualite_moyenne': 8.8},
        {'ruche__description': 'Ruche C4 - Montagne', 'total_poids': 88.7, 'qualite_moyenne': 7.5},
    ]
...
def materiel_list(request):
    materiels = [
        {'type': 'Hausse', 'designation': 'Hausse Dadant 9 cadres', 'date_ajout': '2023-03-15', 'statut': 'Neuf', 'duree_vie': 60},
        {'type': 'Cadre', 'designation': 'Cadre de corps ciré', 'date_ajout': '2023-02-10', 'statut': 'Bon', 'duree_vie': 24},
        {'type': 'Extracteur', 'designation': 'Extracteur 4 cadres manuel', 'date_ajout': '2021-07-01', 'statut': 'Usé', 'duree_vie': 120},
    ]
...
def stock_consommables(request):
    consommables = [
        {'nom': 'Sirop de nourrissement', 'unite': 'Litres', 'stock_actuel': 50, 'seuil_alerte': 20, 'statut': 'ok'},
        {'nom': 'Cire gaufrée', 'unite': 'Feuilles', 'stock_actuel': 80, 'seuil_alerte': 100, 'statut': 'alerte'},
        {'nom': 'Acide oxalique', 'unite': 'Grammes', 'stock_actuel': 15, 'seuil_alerte': 50, 'statut': 'critique'},
    ]
...
def maintenance_planning(request):
    tasks = [
        {'description': 'Nettoyage des planchers - Ruche A1, A2, A3', 'date_prevue': '2023-11-25', 'statut': 'À faire'},
        {'description': 'Changement des cadres de corps - Ruche B1', 'date_prevue': '2023-12-01', 'statut': 'À faire'},
        {'description': 'Révision extracteur', 'date_realisation': '2023-10-15', 'statut': 'Terminé'},
    ]
...
def alertes_penurie(request):
    alertes_stock = [
        {'message': "Cire gaufrée : Stock actuel (80 feuilles) est en dessous du seuil d'alerte (100 feuilles).", 'level': 'warning'},
        {'message': "Acide oxalique : Stock actuel (15g) est très en dessous du seuil d'alerte (50g).", 'level': 'danger'},
    ]
    alertes_maintenance = [
